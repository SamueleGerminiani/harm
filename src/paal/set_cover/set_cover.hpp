...
#include "budgeted_maximum_coverage.hpp"
#include "utils/accumulate_functors.hpp"
#include "utils/functors.hpp"
#include "utils/type_functions.hpp"
...
using set_range_cost_t = pure_result_of_t<GetCostOfSet(
 ...
 */
template <typename SetRange, class GetCostOfSet,
          class GetElementsOfSet, class OutputIterator,
          class GetElementIndex>
auto set_cover(SetRange &&sets, GetCostOfSet set_to_cost,
               GetElementsOfSet set_to_elements,
               OutputIterator result, GetElementIndex get_el_index) {
  using set_cost =
      typename detail::set_range_cost_t<SetRange, GetCostOfSet>;
  //TODO use sum functor from r=Robert commit
  auto cost_of_all_sets =
      accumulate_functor(sets, set_cost{}, set_to_cost);
  set_cost cost_of_solution{};
  budgeted_maximum_coverage(
      sets, set_to_cost, set_to_elements,
      boost::make_function_output_iterator([&](int set) {
        cost_of_solution += set_to_cost(set);
        *result = set;
        ++result;
      }),
      get_el_index, cost_of_all_sets,
      paal::utils::return_one_functor(), 0);
