...
template <typename T>
HEDLEY_CONST CONSTEXPR long double pow10(const T &n) noexcept {
...
/** Compute 10 to the power of n */
template <>
HEDLEY_CONST CONSTEXPR long double pow10(const unsigned &n) noexcept {
  long double multiplicand = n > 0 ? 10 : 0.1, ret = 1;
...
template <> inline DataType type_num<std::nullptr_t>() {
  return DataType::CSV_NULL;
}
template <> inline DataType type_num<std::string>() {
  return DataType::CSV_STRING;
}

CONSTEXPR DataType data_type(csv::string_view in,
                             long double *const out = nullptr);
#endif
...
/** Given a byte size, return the largest number than can be stored in
         ...
         *  byte sizes
         */
template <size_t Bytes> CONSTEXPR long double get_int_max() {
...
/** Given a byte size, return the largest number than can be stored in
         ...
         */
template <size_t Bytes> CONSTEXPR long double get_uint_max() {
  static_assert(Bytes == 1 || Bytes == 2 || Bytes == 4 || Bytes == 8,
                "Bytes must be a power of 2 below 8.");
...
/** Given a pointer to the start of what is start of
         ...
         *  parse the exponent
         */
HEDLEY_PRIVATE CONSTEXPR DataType _process_potential_exponential(
    csv::string_view exponential_part, const long double &coeff,
...
/** Given the absolute value of an integer, determine what numeric type
         *  it fits in
         */
HEDLEY_PRIVATE HEDLEY_PURE CONSTEXPR DataType
