...
TEST_CASE("Test CSVRow Interator", "[test_csv_row_iter]") {
  ...
  SECTION("Range Based For") {
    ...
    for (auto &field : row) {
      if (i == 0)
        REQUIRE(field.get<>() == "123");
      else if (i == 1)
        REQUIRE(field.get<>() == "234");
      else
        REQUIRE(field.get<>() == "345");
...
TEST_CASE("Basic CSVReader Iterator Test", "[read_ints_iter]") {
  ...
  std::vector<std::string> col_names = {"A", "B", "C", "D", "E",
                                        "F", "G", "H", "I", "J"};
  size_t i = 1;
...
TEST_CASE("CSVReader Iterator + std::max_elem", "[iter_max_elem]") {
  ...
  auto wage_finder = [](CSVRow &left, CSVRow &right) {
    return (left["Total Wages"].get<double>() <
            right["Total Wages"].get<double>());
  };
