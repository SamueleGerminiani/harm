...
TEST_CASE("Test Parse Flags", "[test_parse_flags]") {
  REQUIRE(internals::make_parse_flags(',', '"')[162] ==
          internals::ParseFlags::QUOTE);
}
...
TEST_CASE("Test Empty Field", "[read_empty_field]") {
  ...
  CSVRow row;
  rows.read_row(row);
  REQUIRE(vector<string>(row) == vector<string>({"123", "", "456"}));
}

//! [Parse Example]
TEST_CASE("Test Escaped Quote", "[read_csv_quote]") {
  // Per RFC 1480, escaped quotes should be doubled up
  auto csv_string = GENERATE(
      as<std::string>{},
      ("A,B,C\r\n" // Header row
       "123,\"234\"\"345\",456\r\n"
...
TEST_CASE("Test Whitespace Trimming", "[read_csv_trim]") {
  ...
  SECTION("Parse Test") {
    CSVFormat format;
    format.header_row(0).trim({'\t', ' '}).delimiter(',');
...
TEST_CASE("Test Whitespace Trimming w/ Empty Fields") {
  ...
  SECTION("Parse Test") {
    CSVFormat format;
    format.column_names({"A", "B", "C"}).trim({' ', '\t'});
...
123,345,678,)");
  ...
  REQUIRE(reader.get_col_names() ==
          std::vector<std::string>({"A", "B", "C", ""}));

  reader.read_row(first_row);
  REQUIRE(std::vector<std::string>(first_row) ==
          std::vector<std::string>({"123", "345", "678", ""}));
}
