...
int main(int arg, char *argv[]) {
  ...
  for (size_t i = 0; ((int)assStrs.size() - evaluated) > 0; i++) {
    ...
    if (clc::ve_technique == "sr") {
      ...
      csv::CSVReader reader(clc::ve_varList);
      csv::CSVRow row;
...
void parseCommandLineArguments(int argc, char *args[]) {
  ...
  if (result.count("var-list")) {
    clc::ve_varList = result["var-list"].as<std::string>();
  }

  messageErrorIf(clc::ve_technique == "vbr" && !result.count("var-map"),
                 "No var-map specified");
