...
size_t findCommonPrefixLength(VCDScope *vcd_scope) {
...
VCDScope *findScope(VCDFile *vcd_trace, const std::string &scope) {
  std::deque<VCDScope *> scopes;
  std::vector<std::string> scopeName;
...
Trace *VCDtraceReader::readTrace(const std::string file) {
  ...
  if (vcd_trace) {
    ...
    messageError("Parse Failed");
  }

  //   debug
  //     auto p = trace->getLogicVariable("TrojanCounter[1]");
  //     std::ofstream myfile;
  //     myfile.open ("TrojanCounter.txt");
  //     for (size_t i = 0; i <trace->getLength() ; i++) {
  //            myfile<<"L"<<i<<" "<<p->evaluate(i)<<"\n";
  //    }
  //       myfile.close();
  //
  //   std::ofstream myfile;
  //   myfile.open("wb_dat_o.txt");
  //   for (int i = 0; i < 100; ++i) {
  //   std::stringstream ss;
  //
  //    for (int j = 31; j >=0; j--) {
  //
  //      auto p =
  //          trace->getBooleanVariable("wb_dat_o[" + std::to_string(j) + "]");
  //      ss << std::bitset<1>(p->evaluate(i));
  //    }
  //    myfile<<std::stoull(ss.str(),nullptr,2);
  //   myfile << "\n";
  //  }
  //   myfile.close();
