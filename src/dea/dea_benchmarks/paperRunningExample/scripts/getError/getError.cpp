...
std::vector<std::vector<unsigned int>> getValues(const std::string &csvFile,
...
unsigned int getMaxDiff(std::vector<unsigned int> &g_field, std::vector<unsigned int> &f_field) {
    unsigned int max = std::numeric_limits<unsigned int>::lowest();
    for (size_t i = 0; i < g_field.size(); i++) {
        max = std::max((unsigned int)std::abs((int)(g_field[i] - f_field[i])), max);
    }
...
double getError(std::vector<std::vector<unsigned int>> &g,
    ...
    for (int i = 0; i < nFields; ++i) {
        ...
        for (int j = 0; j < nValues; ++j) {
            errField += (g[i][j] == 0.f && f[i][j] == 0.f ? 0.f : (double)(std::abs(((long long int)g[i][j] - (long long int)f[i][j]))) / (double)std::max(g[i][j],f[i][j]));
//            std::stringstream ss;
