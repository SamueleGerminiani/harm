...
TracePtr parseTrace(std::string path) {
...
std::vector<TemplateImplicationPtr>
parseAssertions(std::vector<std::string> assStrs,
                const TracePtr &trace, size_t start, size_t n) {
  std::vector<TemplateImplicationPtr> ret;
#if enPB_ve
  progresscpp::ParallelProgressBar pb;
  pb.addInstance(
      0, "Parsing assertions...",
      (start + n <= assStrs.size() ? n : assStrs.size() - start), 70);
#endif
  for (size_t i = start; i < start + n && i < assStrs.size(); i++) {
    TemplateImplicationPtr ass = hparser::parseTemplateImplication(
        assStrs[i], trace, harm::DTLimits(), 0);

    messageErrorIf(!ass->assHoldsOnTrace(Location::AntCon),
                   "Assertion '" +
...
void getDiff(std::unordered_map<std::string, Diff> &idToDiff,
  ...
  for (auto &s : ids) {
    ...
    if (!std::filesystem::exists(fn)) {
      //simulate
      systemCustom("bash " + clc::ve_shPath + " " + s + " " +
                   clc::ve_ftPath + " 0 " + std::to_string(threadID) +
...
void cluster_with_kmeans(
  ...
  std::map<size_t, std::vector<std::pair<std::string, double>>>
      clusters_score;
  ...
  for (auto &v_s : scores) {
    auto it = std::find_if(
        clusters_range.begin(), clusters_range.end(),
        [&v_s](const std::pair<double, double> &e) {
          return v_s.second <= e.second && v_s.second >= e.first;
        });
    messageErrorIf(it == clusters_range.end(),
                   "Could not find range");
    clusters_score[std::distance(clusters_range.begin(), it)]
        .emplace_back(v_s.first, v_s.second);
  }
