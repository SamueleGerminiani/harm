...
public:
  ...
  enum {
    RuleFile = 0, RuleLogic = 1, RuleLogicAtom = 2, RuleLogicConstant = 3, 
    RuleLogicVariable = 4, RuleNumeric = 5, RuleNumericAtom = 6, RuleNumericConstant = 7, 
    RuleNumericVariable = 8, RuleVariable = 9, RuleRelop = 10
  };

  explicit metricParser(antlr4::TokenStream *input);

  metricParser(antlr4::TokenStream *input, const antlr4::atn::ParserATNSimulatorOptions &options);

  ~metricParser() override;
