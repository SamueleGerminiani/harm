...
( "force-int", "force all logic (4-value) types to be treated as integers (x and z will be treated as 0)")
...
( "reset", "propositional expression that signals an active reset when it evaluates to true on the input traces", cxxopts::value<std::string>(), "<String>")
...
( "ddd", "Enables dumping of debug data", cxxopts::value<std::string>(), "<DIRECTORY>")
( "interactive", "enable interactive assertion ranking")
( "dont-print-ass", "do not print the mined assertions")
( "check-dump-eval", "for each 'check' assertion, dump the evaluation on the input traces of antecedent, consequent, shift (each assertion is dumped to a different file)", cxxopts::value<std::string>(), "<DIRECTORY>")
